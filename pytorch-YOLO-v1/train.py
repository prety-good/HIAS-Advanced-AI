...
train_dataset = yoloDataset(root=file_root,list_file=['voc2012.txt','voc2007.txt','voc2007test.txt'],train=True,transform = [transforms.ToTensor()] )
...
for epoch in range(num_epochs):
    ...
    for i,(images,target) in tqdm(enumerate(train_loader), desc=f"epoch:{epoch}/{num_epochs}", total=len(train_loader)):
    ...
    if (epoch+1 ) %10 == 0:
        torch.save(net.state_dict(),f'./weights/yolov1_{epoch}.pth')
    torch.save(net.state_dict(),f'./weights/yolov1_{epoch}.pth')
